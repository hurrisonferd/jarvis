...
frontend, memory/operations/scripts/CI, canon/architecture. Live-verified against Supabase where
...
`core/supabase/functions/jarvis-mcp/index.ts` — `jarvis_node_register_key` (~678) and the Grid
...
(`operations/scripts/jarvis-session-end.py:75`) + fire-and-forget embedding (`mnemos-store`). Small,
...
- **Gold Law numbering drift:** `CLAUDE.md` uses GL2/5/6/7/**10**; `core/JarvisMain/Architecture/constraints.md`
