...
3. Apply core/supabase/migrations/ in order
...
# Check JVE locally
python3 core/JarvisMain/yggdrasil/tools/seed.py
python3 core/JarvisMain/yggdrasil/tools/validate.py
```
...
| Artifact | Location |
|----------|----------|
| Canon structure | `core/JarvisMain/Architecture/canon/` |
| Edge function source | `core/supabase/functions/<name>/` |
| DB migrations | `core/supabase/migrations/` |
| MCP tool mirrors | `core/JarvisMain/Connectors/JarvisMCPSupabase/tools/` |
| Rebuild seed (private) | `Jarvis-Private/rebuild/JARVIS_REBUILD_SEED.md` |
| Secrets | Supabase env vars + Jarvis-Private |
| Chaos state | `memory/chaos/` (never commit) |
