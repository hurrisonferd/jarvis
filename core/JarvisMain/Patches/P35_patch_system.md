...
- **Four overlapping surfaces** — `memory/audit/patch_ledger.json` (canonical),
  `logs/patch-log.json` (mirror), `memory/mnemos/context/patches.json` (empty daily
...
**Source of truth:** `memory/audit/patch_ledger.json`. Sole writer:
`operations/scripts/jarvis-patch.py`. `logs/patch-log.json` is regenerated from it as a
...
- `operations/scripts/jarvis-patch.py open|add|exec|defer|show|list` — the writer.
