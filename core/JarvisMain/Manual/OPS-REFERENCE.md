...
created: 2026-06-25
updated: 2026-06-25
source: core/JarvisMain/Manual/OPS-REFERENCE.md
...
Full specs: `core/JarvisMain/yggdrasil/jfs/JFS-SPEC.md`, `JNL-GRAMMAR.md`, `JNS-FORMAT.md`, `JSL-SPEC.md`, `JMS-SPEC.md`, `LAL-SPEC.md`.
...
| JSL | Structural Layer | Folder hierarchy: `core/JarvisMain/` (MAIN tier) / `JarvisSide/` (SIDE tier) |
...
Full token table: `core/JarvisMain/yggdrasil/jfs/jnl-grammar.md`

### File Naming (JNS)

`<PROJECT><TYPE>-<MMDDYY>-<NNNN>-<SUBJECT>.md` — e.g. `JARV-AUTO-062526-0001-system-manual.md`
Project codes: `core/JarvisMain/yggdrasil/jfs/project-codes.json`

### Repo Structure (JSL)

```
core/JarvisMain/           MAIN tier: god systems + canonical knowledge
...
JarvisSide/           SIDE tier: periphery
  ...
  Breakthroughs/
  Archive/
Scripts + core/supabase/ + .github/ + memory/chaos/ + memory/mnemos/ + memory/audit/ stay at root (live runtime)
```

### Intake → Commit Loop

```
memory/intake/          → add request
...
Full spec: `core/JarvisMain/Architecture/JMMS-SPEC.md`
...
- KRONOS fold: `core/supabase/functions/kronos-fold/` — daily cron fires JSTM → JHTM

### Bounded Autonomy — Session Close (GL6)

Spec: `core/JarvisMain/Manual/Operations/ARCH-GOV-AUTO-0001-062426.md`

On session close: `session_close` tool scans JSTM memories. If any lack a fold receipt (never promoted), writes a HOLD artifact to `core/JarvisMain/Implementation/tasks/`. Never silent exits. Governed Autonomy Contract says: *"any node operating under a governed autonomy contract MUST write a handoff artifact if it does not reach completion."*
...
| What | Path |
|------|------|
| Yggdrasil kernel | `core/JarvisMain/yggdrasil/` |
| JD entries | `core/JarvisMain/yggdrasil/jd/entries/` |
| Connectors (65 tools) | `core/JarvisMain/Connectors/` |
| Seed tool | `core/JarvisMain/yggdrasil/tools/seed.py` |
| JVE validator | `core/JarvisMain/yggdrasil/tools/validate.py` |
| MCP (jarvis-mcp) | `core/supabase/functions/jarvis-mcp/index.ts` |
| MCP (jarvis-action) | `core/supabase/functions/jarvis-action/index.ts` |
| KRONOS fold | `core/supabase/functions/kronos-fold/index.ts` |
| GitHub Actions | `.github/workflows/` |
| Intact ops specs | `core/JarvisMain/Manual/Operations/` |
| Audit log | `memory/audit/audit_log/` |
| Governed workflow checklist | `operations/scripts/jarvis-session-start.sh` (session start hook) |
| Honest Answering Contract | `core/JarvisMain/Architecture/specs/IMPL-HON-SPEC-0001.md` |
| Pre-Act Verification Contract | `core/JarvisMain/Architecture/specs/pre-act-verification-contract.md` |
| Throughput Posture (HALO) | `core/JarvisMain/Architecture/specs/throughput-posture.md` |
| MIMIR routing table | `core/JarvisMain/Architecture/specs/GOVKRSPEC-061326-0001-KNOWLEDGE-ROUTING-INDEX.md` |
