...
function buildServer(req: Request): McpServer {
  ...
  server.registerTool(
    ...
    async ({ input, prior_reply, context }) => {
      ...
      try {
        ...
        // (plan/decide/memory/audit/expansion/analyze). The council analysis ALWAYS carries
  ...
  server.registerTool(
    "jarvis_mint",
    {
      ...
      inputSchema: {
        ...
        scan_root: z.string().max(80).optional().default("core/JarvisMain/Implementation/task"),
      },
    },
    async ({ domain, system, type, name, definition, purpose, tags, status, scan_root }) => {
      ...
      const reg = await gh(`/contents/${ghPath("core/JarvisMain/yggdrasil/lal/address-registry.json")}?ref=main`);
  ...
  server.registerTool(
    "jarvis_repo_tree",
    {
      ...
      description:
        "List repo file paths at a ref (default main). Filter with prefix (e.g. 'core/JarvisMain/yggdrasil/'). Read-only parity surface: see the actual architecture instead of inferring it from registries.",
      inputSchema: {
  ...
  // Raven generates the keypair OFF-SYSTEM (operations/scripts/grid_keygen.mjs) and signs the
  ...
  server.registerTool(
    ...
    async ({ public_key, identity_cert, owner }) => {
      ...
      if (!ok) {
        return text({
          registered: false,
          error: "identity_cert did not verify against the rebuilt assertion + public_key. Sign these EXACT bytes with the matching Ed25519 private key (operations/scripts/grid_keygen.mjs keygen).",
          node_id: NODE_ID,
          assertion,
        });
      }
    ...
    async ({ page }) => {
      ...
      if (wantRaw === "rehydrate" || wantRaw === "omni") {
        const grab = async (f: string) => {
          const r = await gh(`/contents/${ghPath("core/JarvisMain/yggdrasil/lal/" + f)}?ref=main`);
      ...
      if (LENS_FILES[wantRaw]) {
        const lf = await gh(`/contents/${ghPath("core/JarvisMain/yggdrasil/lal/" + LENS_FILES[wantRaw])}?ref=main`);
      ...
      const res = await gh(`/contents/${"core/JarvisMain/yggdrasil/lal/GRIMOIRE.md".split("/").map(encodeURIComponent).join("/")}?ref=main`);
  ...
  server.registerTool(
    ...
    async ({ who }) => {
      const paths: Record<string, string> = {
        jarvis: "core/JarvisMain/Architecture/identity/jarvis/index.md",
        ayre: "core/JarvisMain/Architecture/identity/ayre/index.md",
        argent: "core/JarvisMain/Architecture/identity/argent/argent-profile.md",
        relational: "core/JarvisMain/Architecture/identity/relational/relational-profile.md",
        raven: "core/JarvisMain/Architecture/identity/raven/index.md",
      };
  ...
  server.registerTool(
    ...
    async ({ summary_only }) => {
      const res = await gh(`/contents/${"core/JarvisMain/yggdrasil/lal/global-mirror.json".split("/").map(encodeURIComponent).join("/")}?ref=main`);
  ...
  server.registerTool(
    ...
    async () => {
      ...
      const mraw = await readGh("core/JarvisMain/yggdrasil/lal/global-mirror.json");
      ...
      return text({
        ...
        wiring_map: (await readGh("core/JarvisMain/yggdrasil/lal/WIRING-MAP.md")) ?? "unreachable",
        health: (await readGh("core/JarvisMain/yggdrasil/lal/HEALTH.md")) ?? "unreachable",
      });
    },
  );
  ...
  server.registerTool(
    ...
    async () => {
      const r = await gh(`/contents/${"core/JarvisMain/yggdrasil/lal/PINCH.md".split("/").map(encodeURIComponent).join("/")}?ref=main`);
  ...
  const READS: Record<string, () => Promise<unknown>> = {
    state: async () => { const raw = await lensFile("core/JarvisMain/yggdrasil/lal/global-mirror.json"); try { const m = JSON.parse(raw); return { freshness: m.freshness, summary: m.summary }; } catch { return "unreachable"; } },
    pinch: () => lensFile("core/JarvisMain/yggdrasil/lal/PINCH.md"),
    health: () => lensFile("core/JarvisMain/yggdrasil/lal/HEALTH.md"),
    wiring: () => lensFile("core/JarvisMain/yggdrasil/lal/WIRING-MAP.md"),
    ...
    sensory: () => lensFile("core/JarvisMain/Architecture/identity/sensory/SENSORY-0001-062525-THE-SENSES.md"),
  };
  ...
  server.registerTool(
    ...
    async () => {
      ...
      try {
        const r = await gh(`/contents/${"core/JarvisMain/yggdrasil/lal/address-registry.json".split("/").map(encodeURIComponent).join("/")}?ref=main`);
  ...
  server.registerTool(
    ...
    async () => {
      ...
      const who = await (async () => {
        const r = await gh(`/contents/${"core/JarvisMain/Architecture/identity/raven/raven-profile.md".split("/").map(encodeURIComponent).join("/")}?ref=main`);
        if (!r.ok) return null;
        const d = await r.json() as { content?: string };
        return typeof d.content === "string" ? atob(d.content.replace(/\n/g, "")) : null;
      })().catch(() => null);
  ...
  server.registerTool(
    ...
    async ({ query, mode }) => {
      ...
      // GitHub file content
      if (primary?.jnl) {
        const entryPath = `core/JarvisMain/yggdrasil/jd/entries/${primary.jnl}.md`;
        try {
...
// Force rebuild - 2026-07-23T20:51:34Z
