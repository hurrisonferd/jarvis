...
├── Spawner (Node3D)               attach scripts/spawner.gd
└── HUD (CanvasLayer)              score + multiplier labels (later)
```
Make a **Ball** scene: `RigidBody3D` (attach `scripts/ball.gd`) + `MeshInstance3D`
(SphereMesh) + `CollisionShape3D` (SphereShape). Save as `ball.tscn`, assign it to
the Spawner's `ball_scene`.

Multiplier wells: `Area3D` + `scripts/multiplier_zone.gd`, dropped into the trenches.

## The trap to avoid
Leave **Continuous CD OFF** on the ball — it breaks `PhysicsMaterial.bounce` in Godot.

## Files
- `scripts/ball.gd` — the ball; bounce material set in code + haptic on impact.
- `scripts/spawner.gd` — tap → raycast to floor → drop a ball (the tap spawner, done right).
- `scripts/multiplier_zone.gd` — trench wells that score, multiply, and pop the ball.
