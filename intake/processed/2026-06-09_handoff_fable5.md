...
## Structure
`JarvisMain/` (core: god_systems + Architecture/Audit/Implementation/Patches/Connectors) ·
`JarvisSide/` (periphery: Projects/Ideas/Breakthroughs/Archive/Deprecated) ·
`yggdrasil/` (substrate, at root) · runtime at root (`supabase/ docs/ .github/ scripts/`).

## Operational truths (don't relearn the hard way)
- **Always cut branches with `/newbranch <name>`** (`scripts/newbranch.sh`). Branching off a
...
- Don't redefine the 27 God Systems. Don't commit `chaos/*` or `.env`.

## Open decisions (Raven's call)
1. **Move `yggdrasil/` into `JarvisMain/`** — Raven wants it; recommended next build (tool
