...
function vehicleBody(store){
  ...
  const v=inst.vehicle||{},p=inst.performance||{},c=inst.capacity||{},proof=inst.proof||{},safe=inst.safety||{},loop=inst.god_loop||{},gf=inst.godframe||{},vasc=inst.vascular||{},dc=inst.damage_control||{};
  ...
  return `<div class="panel">
    ...
    <div>MAD MAX ${Number(dc.upgrade_count||0)} UPGRADES · LIMP MIN ${Number(dc.minimum_limp_home_godframes||0)}</div>
    <div>RECOVERY ${esc((dc.recovery_ladder||[]).join('>')||'UNKNOWN')}</div>
    <div>POWER/SPOOL ${dc.priority_spool_binding===true?'BOUND':'UNKNOWN'} · THERMAL ${dc.thermal_derate===true?'DERATE':'UNKNOWN'} · SHED ${dc.queue_load_shed===true?'BOUND':'UNKNOWN'}</div>
    <div>AUTO BYPASS ${dc.automatic_bypass===false?'NO':'UNKNOWN'} · AUTO EFFECTS ${dc.automatic_effects===false?'NO':'UNKNOWN'}</div>
