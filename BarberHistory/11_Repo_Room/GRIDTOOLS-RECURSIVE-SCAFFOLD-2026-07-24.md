...
scripts/ACTIVE = public current utility shelf
scripts/INACTIVE = parked public utility shelf
supabase/functions/* = remote MCP/action surface
JarvisMain/yggdrasil/tools = canon maintenance toolkit
...
| `supabase/functions` | 28 private-tree hits | Remote Edge Function/MCP/action surface. |
...
```text
scripts/jarvis_heartbeat.py
scripts/generate_vapid.py
scripts/install_gbrain.ps1
mnemos/mnemos_vector.py
chaos/session_sync.py
supabase/functions/jarvis-mcp/*
supabase/functions/send-push/*
src/diagnostics.ts
```

Public mirror has about `126` script/tool hits under `scripts`, `JarvisMain/yggdrasil/tools`, and `supabase/functions`.
...
| `scripts/ACTIVE` | Public/live repo utility shelf. |
| `scripts/INACTIVE` | Public parked utilities. |
| `scripts/DEPRECATED` | Public replaced tests/tools. |
| `JarvisMain/yggdrasil/tools` | Canon/yggdrasil maintenance toolkit. |
| `supabase/functions` | Remote deployed/action surface. |
...
3. Register public `scripts/ACTIVE`.
