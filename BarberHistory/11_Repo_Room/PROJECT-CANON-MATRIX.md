...
| --- | --- | --- | --- | --- | --- |
| JARVIS MCP | `supabase/functions/jarvis-mcp` | larger `supabase/functions/jarvis-mcp`, `JarvisMain/Connectors` | private `supabase/functions/jarvis-mcp` plus Lilith/private tools | live Supabase + current code for active local; public for history | Build function manifest. |
...
| Grid | current docs/intake | public `grid`, `JarvisMain/Architecture`, `gameboy` | private `Living_Codex/Ego/Grid`, `workspaces/Grid`, GridTools | private + public architecture together | Create Grid Canon Map. |
...
| CodeOS | BarberHistory lost civ map | public `mnemos/knowledge/projects/codeos.md` trace | `workspaces/CodeOS`, `workspaces/Projects/CodeOS`, equations, games | private ghost | Split CodeOS core, Physics, game seeds. |
