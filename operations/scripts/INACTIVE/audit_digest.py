#!/usr/bin/env python3
"""
audit_digest.py — generate weekly and monthly digests from memory/audit/audit_log entries.
Run manually or via CI on Friday evenings / end of month.

Usage:
  python3 operations/scripts/audit_digest.py --week 2026-W26
  python3 operations/scripts/audit_digest.py --month 2026-M06
  python3 operations/scripts/audit_digest.py --week 2026-W26 --dry-run
"""
    ...
    if AUDIT_LOG.is_dir():
        for f in sorted(AUDIT_LOG.glob("*.md")):
            ...
            if f.name not in seen:
                seen.add(f.name)
                yield f
    # Year/week folders: memory/audit/YYYY/*.md + memory/audit/YYYY/Www/*.md
    if WEEK_ROOT.is_dir():
...
def format_week_digest(entries, start, end, week_label, dry_run=False):
    ...
    body_lines.append(f"_Generated {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')} · `operations/scripts/audit_digest.py`_")
