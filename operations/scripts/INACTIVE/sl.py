...
Usage:
  python3 operations/scripts/sl.py --session-start
  python3 operations/scripts/sl.py --session-close "What happened"
  python3 operations/scripts/sl.py --sync-tasks
  python3 operations/scripts/sl.py -b "Did X and Y"
  python3 operations/scripts/sl.py -t BUILD -b "Shipped Z" --commit
...
def build_log(brief, log_type, title, tasks_summary, stream):
    ...
    body += ["---", "*Generated by sl.py · memory/audit/starlogs/*"]
    return "\n".join(body)
...
def main():
    ...
    if args.mimir:
        ...
        print("Full table: core/JarvisMain/Architecture/specs/GOVKRSPEC-061326-0001-KNOWLEDGE-ROUTING-INDEX.md")
        print()
