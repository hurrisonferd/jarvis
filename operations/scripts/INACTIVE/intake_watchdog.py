...
Scans memory/intake/ for unreviewed files. If the backlog exceeds the threshold (default 3),
...
Moves aged-but-reviewed files from memory/intake/ subdirs into memory/intake/processed/
...
def main() -> int:
    ...
    print(f"INTake watchdog: {backlog} unreviewed file(s) in memory/intake/")
    ...
    body = (
        ...
        f"Raven, these items have been in memory/intake/ without moving to `processed/`.\n"
        ...
        f"`operations/scripts/intake_watchdog.py`*"
    )
