...
The companion grows three ways and this wires all three from one call:
  1. the partitioned record  -> memory/mnemos/knowledge/<category>.md (durable, in repo)
  2. the learned ledger       -> memory/mnemos/memories/learned.jsonl (append-only trail)
  3. the live memory          -> prints a ready MNEMOS insert (seed + embed)

Usage:
  python3 operations/scripts/companion_remember.py raven "Raven prefers X over Y"
  python3 operations/scripts/companion_remember.py project:grid "GNPL is the node handshake"
  python3 operations/scripts/companion_remember.py session "Wired the remember loop"
...
def main(argv: list) -> int:
    ...
    print(f"  ledger:  memory/mnemos/memories/learned.jsonl")
