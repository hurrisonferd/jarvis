...
Writes memory and context files to memory/mnemos/ so any model with repo access
can read JARVIS memory without Supabase credentials.

Structure:
  memory/mnemos/memories/recent.json    — last 50 memories (all types)
  memory/mnemos/memories/decisions.json — PROMETHEUS governance decisions
  memory/mnemos/memories/sessions.json  — last 20 session summaries
  memory/mnemos/context/system.json     — Gold Laws, pipeline, God Systems, agents
  memory/mnemos/context/patches.json    — patch status (active + pending only)
  memory/mnemos/index.json              — manifest
...
def write_system_context() -> None:
    write_json(CONTEXT / "system.json", {
        ...
        "memory_architecture": {
            ...
            "sync":   "daily GitHub Action → memory/mnemos/memories/ + memory/mnemos/context/ files",
            "read":   "any model reads memory/mnemos/ files from repo — no credentials needed",
            "fallback": "git log → memory/mnemos/memories/recent.json when Supabase unavailable",
        },
    })
...
def write_index(counts: dict) -> None:
    write_json(MNEMOS / "index.json", {
        ...
        "read_order": [
            "1. memory/mnemos/context/system.json — architecture, Gold Laws, agents (read first)",
            "2. memory/mnemos/memories/recent.json — last 50 memories or git log (Supabase optional)",
            "3. memory/mnemos/memories/decisions.json — approved governance decisions",
            "4. memory/mnemos/memories/sessions.json — session summaries + pipeline observations",
            "5. memory/mnemos/context/patches.json — patch status",
            "6. memory/mnemos/memories/events.jsonl — github issues/PRs",
            "7. memory/mnemos/domains/ — identity profiles (Raven, JARVIS)",
        ],
        "files": {
