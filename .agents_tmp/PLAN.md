...
- **Yggdrasil / JFS / JNL / JSE** — governed in `JarvisMain/yggdrasil/`: tools (`validate.py`, `seed.py`, `grimoire.py`, `dex.py`, `autosort.py`, `extend.py`), registries (`lal/master-index.json`, `lal/address-registry.json`, `lal/tag-registry.json`, `lal/graph.json`), specs (`JFS-SPEC.md`, `jnl-grammar.md`, `jse-schema.md`, `JSS-SPEC.md`, `JMMS-SPEC.md`)
- **Council** — defined in `supabase/functions/jarvis-mcp/council.ts`: TIERS (T0–T9, 27 systems), TIER_WEIGHT, ROLE, COMMENTARY set, LENS_SIGNALS, deliberation logic
- **GRIMOIRE** — `JarvisMain/yggdrasil/lal/GRIMOIRE.md` (230 governed objects, 351 edges, 9 domains) + `JarvisMain/yggdrasil/tools/grimoire.py` (generator)
- **27 God Systems** — 28 README contracts under `JarvisMain/god_systems/T*_*/README.md`; council.ts is the canonical runtime implementation
...
- **MCP connector** — `supabase/functions/jarvis-mcp/index.ts` (Hono + MCP SDK, 30+ tools registered); Supabase edge functions under `supabase/functions/`
- **Memory** — MNEMOS (`supabase/functions/mnemos-*`), JMMS tiers (`jitm`/`jstm`/`jltm`/`jatm`), chaos state in `chaos/` (seed.json excluded from git, `session_sync.py`)
...
2. Writes its findings to `intake/audit-phase-N-*.md`
...
**Reference:** `JarvisMain/yggdrasil/`, `supabase/functions/jarvis-mcp/council.ts`
...
8. Write `intake/audit-phase-1-architecture.md` with:
...
**Reference:** `JarvisMain/god_systems/`, `supabase/functions/jarvis-mcp/council.ts`, `supabase/functions/jarvis-respond/router.ts`

1. List `JarvisMain/god_systems/T*_*/README.md` → should be exactly 28 (including README.md at root)
...
6. Write `intake/audit-phase-2-godsystems.md` with:
...
**Reference:** `supabase/functions/jarvis-mcp/index.ts`, `supabase/functions/`, `JarvisMain/Connectors/JarvisMCPSupabase/`

1. Count `registerTool` calls in `jarvis-mcp/index.ts` → tool count
2. List `supabase/functions/*/index.ts` → edge function list
3. Read `JarvisMain/Connectors/JarvisMCPSupabase/tools/` → doc mirrors for each tool
...
7. Write `intake/audit-phase-3-mcp-tools.md` with:
...
**Reference:** `supabase/functions/mnemos-*/`, `supabase/functions/jarvis-mcp/index.ts`, `chaos/session_sync.py`

1. List `supabase/functions/mnemos-*` → confirm: store, recall, search, embed
...
4. Check `chaos/` exclusion: confirm chaos_seed.json, session_log.json, prometheus_log.json are in .gitignore
5. Read `session_sync.py` → verify session start/end logging works
6. Verify `mnemos/` Supabase table schema (from migrations): content, tags, tier, source columns
7. Write `intake/audit-phase-4-memory.md` with:
...
**Reference:** `JarvisMain/Architecture/`, `supabase/migrations/`, `supabase/functions/jarvis-respond/`

1. List `supabase/migrations/*` → identify: dex_events, jd_entries, jd_proposals, jip_entries schemas
2. Read `JarvisMain/Architecture/specs/dex-council-bridge.md` → verify domain→authority mapping
...
7. Write `intake/audit-phase-5-governance.md` with:
...
**Method:** Read all 5 `intake/audit-phase-*-*.md` files, produce verdict table.
**Reference:** All phase outputs

1. Read `intake/audit-phase-1-architecture.md`
2. Read `intake/audit-phase-2-godsystems.md`
3. Read `intake/audit-phase-3-mcp-tools.md`
4. Read `intake/audit-phase-4-memory.md`
5. Read `intake/audit-phase-5-governance.md`
6. Write `intake/audit-synthesis.md` with:
...
```
# Before Phase 2: assert intake/audit-phase-1-architecture.md exists
# Before Phase 3: assert intake/audit-phase-2-godsystems.md exists
# ...
```

**Final validation:**
- `intake/audit-synthesis.md` exists with a verdict table
