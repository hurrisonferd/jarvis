# audit/task-log — OpenHands Task Tracker Git Log
...
`scripts/audit_task_log.py` — log + commit task state
`scripts/audit_task_log.py --snapshot --commit "session close"` — session close
`scripts/audit_task_log.py --close JNL-REF` — close a task by JNL
