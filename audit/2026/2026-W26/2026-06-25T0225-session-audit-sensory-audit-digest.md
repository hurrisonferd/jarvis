...
- `supabase/functions/jarvis-mcp/index.ts` updated: AINZ now runs READ steps in order
  `state → keel → memory → sensory → pinch`
- `sensory` READ: loads `JarvisMain/Architecture/identity/sensory/SENSORY-0001-062525-THE-SENSES.md`
- 4 mirrors refreshed: `jarvis_ainz.md`, `jarvis_eyes.md`, `jarvis_listen.md`, `jarvis_media_view.md`

### 2. Audit digest structure + tooling
- `scripts/audit_digest.py`: weekly (`--week YYYY-Www`) and monthly (`--month YYYY-MM`)
  digest generation from `audit/audit_log/` entries
- `--ingest`: one-time migration of flat entries into `audit/YYYY/Www/` folder tree
- `--dry-run`: preview without writing
- 3 existing sessions ingested: W26 (ISO week 26, 2026-06-24/25 all fall in W26)
- `audit/2026/2026-W26-WEEKLY-DIGEST.md` + `audit/2026/2026-06-MONTHLY-DIGEST.md` generated
...
- kronos-fold initially failed: `esm.sh` returned 522 for `@supabase/supabase-js@2` (transient CDN error)
