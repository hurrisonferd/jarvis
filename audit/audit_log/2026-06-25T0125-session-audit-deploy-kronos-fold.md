...
- `supabase/functions/kronos-fold/index.ts` deployed (60.9kB).
- `supabase/functions/kronos-fold/deno.json` created (uses `supabase` and `postgres` imports).
- All 4 functions now on cloud: `jarvis-mcp`, `jarvis-dex`, `jarvis-action`, `kronos-fold`.

### 2. Migration — JMMS/JSE tier integration
- `supabase/migrations/20260624_jmms_jse_tier_integration.sql` created.
...
- Trigger paths expanded to include `.github/workflows/deploy-edge-functions.yml`,
  `supabase/functions/kronos-fold/**`, and `supabase/migrations/**`.
