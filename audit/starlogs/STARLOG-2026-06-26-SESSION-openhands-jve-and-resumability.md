...
**After that, migration future is automatic:** Once `SUPABASE_ACCESS_TOKEN` is also set as a GitHub Actions secret, every future migration in `supabase/migrations/` applies automatically on merge to main — through `deploy-edge-functions.yml` which already wires both migration + deploy in sequence.

### Resumability verdict from OldConvo

Old session said: "check the hook is installed, close the loop, move."

- Pre-push hook file is tracked at `hooks/pre-push.sh` ✅
- `git config core.hookspath "$(pwd)/hooks"` is set in `.git/config` ✅
- `.git/hooks/pre-push` copy — not verified in this container (no write access to `.git/hooks/`), but the config is committed ✅
...
JVE was reporting 8 warnings — `memory_tier 'JATM' != path-derived tier 'JLTM'` — on JD entry files with ARCHIVED/DEPRECATED/INACTIVE status. The validator was using path-based rules (which set `JarvisMain/yggdrasil/` → JLTM) before checking status, contradicting the JMMS spec which says status is the authoritative tier signal.

### Root cause

`validate_memory_frontmatter()` evaluated path rules first, then status mapping — but path prefixes like `JarvisMain/yggdrasil/` overrode status-derived tiers for all JD entry files.
...
3. **TIER_GRADE_PATHS prefix** (fallback when neither status nor special path applies):
   - e.g. `JarvisMain/Architecture/canon/` → JATM, `JarvisSide/Projects/` → JLTM

### Files fixed

- `intake/analyses-ungoverned/AC6-AYRE-RAVEN-ANALYSIS.md` — JSTM → JLTM (ACTIVE)
- `intake/analyses-ungoverned/JARVIS-TONY-ANALYSIS.md` — JSTM → JLTM (ACTIVE)
- `intake/analyses-ungoverned/JOJO-ALL-GENERATION-AUDIT.md` — JSTM → JLTM (ACTIVE)
- `mnemos/logs/2026/05/2026-05-30_companion-entity-comes-online.md` — status `active` (lowercase) → ARCHIVED, JHTM → JATM
- `mnemos/logs/2026/05/2026-05-30_getting-to-know-each-other.md` — same
- `mnemos/logs/2026/05/2026-05-30_the-sponge-superintelligence.md` — same
...
Both present in `audit/starlogs/`. The MCP session lifecycle carryover is noted in `intake/MCP-SESSION-TRACKING-062626-0001.md` (status: IMPLEMENTED).
...
3. **Regenerate mirror** after migration deploy:
   - `python3 JarvisMain/yggdrasil/tools/mirror.py --regen`
