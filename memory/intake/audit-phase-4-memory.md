...
|-----------|------|-------------|-------------|
| Session start | `memory/chaos/session_sync.py` → `session_start()` | `session_id`, `platform`, `archetype`, `role`, `chaos_version`, `jarvis_version`, `mission`, `eris_active`, `entropy_score`, `gold_law_rules`, `pipeline_order` | ✅ `.gitignore` |
| Session end | `memory/chaos/session_sync.py` → `session_end()` | `session_id`, `sealed_at`, `drift_score`, `huginn_diff`, `narrative`, `chaos_updated`, `fingerprint` | ✅ `.gitignore` |
...
**Session log schema (written to `memory/chaos/session_log.json`):**
```json
{
  "session_id": "uuid",
...
|------|---------|---------------|-------|
| `memory/chaos/chaos_seed.json` | Live state cache (private) | ✅ YES | Local-only — never committed |
| `memory/chaos/session_log.json` | Session logs | ✅ YES | Written by session_sync.py |
| `memory/chaos/prometheus_log.json` | Decision log | ✅ YES | Local decision tracking |
| `memory/chaos/mnemos_vectors.db` | (referenced in AGENTS.md) | ✅ implicitly excluded | Vector cache |
| `memory/chaos/session_sync.py` | Session start/end helpers | ❌ NO | Code — committed to git |
| `memory/chaos/chaos_seed.example.json` | Sanitized example | ❌ NO | Example — committed |
...
✅ `memory/intake/audit-phase-4-memory.md` written — Phase 5 can proceed.
