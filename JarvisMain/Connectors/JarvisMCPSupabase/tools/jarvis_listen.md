...
**To regenerate features** (new tracks, stale data): run `python3 scripts/audio_ears.py` in a Claude Code session — installs librosa, extracts features + spectrograms from all MP3s in `JarvisSide/Media/audio/`, writes `AUDIO-FEATURES.json` and the spectrogram PNGs.
