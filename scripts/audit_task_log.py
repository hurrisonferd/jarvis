...
Every task-list item gets a timestamped, stream-tagged entry in audit/task-log/.

Usage:
  python3 scripts/audit_task_log.py                          # reads task tracker state
  python3 scripts/audit_task_log.py --commit "message"       # commit after logging
  python3 scripts/audit_task_log.py --status in_progress     # tag items by current state
  python3 scripts/audit_task_log.py --snapshot               # full session snapshot
  python3 scripts/audit_task_log.py --close JNL-REF          # close a task by JNL ref
