...
function buildServer(req: Request): McpServer {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // FOLD — governed JLTM compression with receipt (ARCH-JMMS-SPEC-0001)
  // Compresses old JLTM memories → summary + archives originals + JATM receipt
  // ═══════════════════════════════════════════════════════════════════════════

  server.registerTool(
    "jarvis_fold",
    {
      title: "FOLD — governed memory compression",
      description:
        "Compress old JLTM memories into summaries with a governed receipt. `dry_run:true` (default) previews what would be folded — count, age range, topics. `dry_run:false` executes: groups old memories by tags, writes a summary for each group back to JLTM (tagged `fold`), archives originals (tagged `folded`), and writes a receipt to JATM documenting what was lost and why. AEGIS-gated when not dry-run.",
      inputSchema: {
        older_than_days: z.number().int().min(1).max(365).optional().default(14)
          .describe("Compress memories older than this many days"),
        domain: z.string().max(40).optional()
          .describe("Scope to a memory domain (e.g., 'musicos')"),
        dry_run: z.boolean().optional().default(true)
          .describe("true=preview only, false=execute fold (AEGIS-gated)"),
      },
    },
    async ({ older_than_days, domain, dry_run }) => {
      const cutoff = new Date(Date.now() - older_than_days * 86400000).toISOString();
      // Fetch JLTM memories older than cutoff
      let q = `mnemos_memories?select=id,text,tags,source_type,timestamp&tags=cs.{jltm}&timestamp=lt.${cutoff}&order=timestamp.asc&limit=200`;
      if (domain) q += `&tags=cs.{${domain.toLowerCase()}}`;
      // Exclude already-folded and jatm memories
      const rawRows = await rest(q).catch(() => []) as any[];
      if (!Array.isArray(rawRows)) return text({ status: "ERROR", error: "Could not fetch memories" });
      const rows = rawRows.filter((r: any) => {
        const tags = (r.tags ?? []).map((t: string) => t.toLowerCase());
        return !tags.includes("folded") && !tags.includes("jatm") && !tags.includes("fold");
      });

      if (rows.length === 0) {
        return text({ status: "CLEAN", message: `No JLTM memories older than ${older_than_days} days to fold.`, domain: domain ?? null });
      }

      // Group by primary tag (first non-tier tag)
      const groups: Record<string, any[]> = {};
      for (const r of rows) {
        const tags = (r.tags ?? []).map((t: string) => String(t).toLowerCase());
        const topic = tags.find((t: string) => !["jltm", "jstm", "jitm", "jatm", "fold", "folded"].includes(t)) ?? "general";
        (groups[topic] ??= []).push(r);
      }

      const preview = Object.entries(groups).map(([topic, mems]) => ({
        topic,
        count: mems.length,
        oldest: mems[0]?.timestamp,
        newest: mems[mems.length - 1]?.timestamp,
        sample: mems[0]?.text?.slice(0, 120),
      }));
      ...
      if (dry_run) {
        return text({
          ...
          domain: domain ?? null,
          cutoff,
          instruction: "Review this preview. To execute, call jarvis_fold with dry_run:false (AEGIS-gated).",
        });
      }

      // AEGIS gate for actual fold
      if (!writeAuthorized(req)) return heldForApproval("memory.fold", { total: rows.length, groups: preview.map(g => g.topic) }, req);
      ...
      for (const [topic, mems] of Object.entries(groups)) {
        ...
        const texts = mems.map((m: any) => m.text).filter(Boolean);
        const summary = `[FOLD ${new Date().toISOString().slice(0, 10)}] ${topic} (${texts.length} memories, ${mems[0]?.timestamp?.slice(0, 10)} → ${mems[mems.length - 1]?.timestamp?.slice(0, 10)}): ${texts.join(" | ").slice(0, 800)}`;

        // Write summary as new JLTM entry tagged 'fold'
        const sumBody = { text: summary, source_type: "fold", tags: ["jltm", "fold", topic], timestamp: new Date().toISOString() };
        await fetch(`${SUPABASE_URL}/rest/v1/mnemos_memories`, {
          method: "POST", headers: { authorization: `Bearer ${SERVICE_KEY}`, apikey: SERVICE_KEY, "content-type": "application/json", Prefer: "return=minimal" },
          body: JSON.stringify(sumBody),
        });
        summaries.push(topic);

        // Archive originals: tag with 'folded', remove 'jltm'
        for (const m of mems) {
          const newTags = (m.tags ?? []).filter((t: string) => t.toLowerCase() !== "jltm").concat(["folded"]);
          await fetch(`${SUPABASE_URL}/rest/v1/mnemos_memories?id=eq.${m.id}`, {
            method: "PATCH", headers: { authorization: `Bearer ${SERVICE_KEY}`, apikey: SERVICE_KEY, "content-type": "application/json", Prefer: "return=minimal" },
            body: JSON.stringify({ tags: newTags }),
          });
          folded.push(m.id);
        }
      ...
      // Write receipt to JATM (immutable record of what was lost)
      const receipt = {
        text: `[FOLD RECEIPT ${new Date().toISOString()}] Folded ${folded.length} JLTM memories older than ${older_than_days} days. Groups: ${summaries.join(", ")}. Domain: ${domain ?? "all"}. Cutoff: ${cutoff}. Archived IDs: ${folded.slice(0, 20).join(", ")}${folded.length > 20 ? `... (+${folded.length - 20} more)` : ""}.`,
        source_type: "fold_receipt",
        tags: ["jatm", "fold_receipt"],
      ...
      return text({
        status: "FOLDED",
        memories_archived: folded.length,
        summaries_written: summaries.length,
        receipt_written: true,
        receipt_tier: "JATM",
        groups: preview,
        note: "Fold complete. Originals archived (tagged 'folded'), summaries in JLTM (tagged 'fold'), receipt in JATM (immutable).",
      });
    },
  );
